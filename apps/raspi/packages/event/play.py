...
def play(e: Event):
    ...
    r = fetch(endpoint=f"event/{e.id}", method='POST', body='start')
    if not r.ok:
    ...
    messages = [{'from': 'Mom', 'text': 'Hello world!'}]

    GPIO.setmode(GPIO.BCM)

    buzzer = Buzzer(melody.notes, melody.beat)
    ...
    r = fetch(endpoint=f"event/{e.id}", method='POST', body='end')
    if not r.ok:
